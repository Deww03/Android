...
public class MainActivity extends Activity {
    ...
    private void getData() {
        Thread thread = new Thread(new Runnable() {
            @Override
            public void run() {
                String urlString = "http://192.168.30.224:80/warung/getdata.php";
                try {
            ...
            public void run() {
                try {
                    ...
                    for (int i = 0; i < jsonArray.length(); i++) {
                        ...
                        String persediaan = jsonObject.getString("persediaan");

                        String item = "ID Produk: " + id_produk + "\nNama: " + nama + "\nJenis: " + jenis + "\nHarga: " + harga + "\nStok: " + persediaan;
                        barangList.add(item);
                    }
    ...
    private void deleteData(final String id_produk, final int position) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                String urlString = "http://192.168.30.224:80/warung/hapus.php";
                try {
                    URL url = new URL(urlString);
                    HttpURLConnection urlConnection = (HttpURLConnection) url.openConnection();
